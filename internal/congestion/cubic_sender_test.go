...
var _ = Describe("Cubic Sender", func() {
	...
	AckNPacketsLen := func(n int, packetLength protocol.ByteCount) {
		...
		for i := 0; i < n; i++ {
			ackedPacketNumber++
			sender.OnPacketAcked(ackedPacketNumber, packetLength, bytesInFlight, time.Time{})
		}
	...
	// 	for i := 1; i < int(protocol.DefaultMaxCongestionWindow/protocol.DefaultTCPMSS); i++ {
	// 		// ackedPackets := []
	// 		sender.MaybeExitSlowStart()
	// 		sender.OnPacketAcked(protocol.PacketNumber(i), 1350, sender.GetCongestionWindow(), clock.Now())
	// 	}
	// 	Expect(sender.GetCongestionWindow() / protocol.DefaultTCPMSS).To(Equal(sender.GetCongestionWindow()))
	// })

	PIt("limit cwnd increase in congestion avoidance", func() {
		...
		AckNPackets(2)
		Expect(savedCwnd + protocol.DefaultTCPMSS).To(Equal(sender.GetCongestionWindow()))
	})
})
